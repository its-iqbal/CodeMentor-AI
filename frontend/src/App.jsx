...
function App() {
  ...
  // State for navigation and history data
  const [currentView, setCurrentView] = useState('new'); // 'new' | 'history'
  const [historyData, setHistoryData] = useState([]);
  const [isLoadingHistory, setIsLoadingHistory] = useState(false);

  // --- UPDATED AUTHENTICATION LOGIC ---
  const [token, setToken] = useState(localStorage.getItem('codementor_token') || null);
  ...
  useEffect(() => {
    ...
    // Define activeToken cleanly inside the effect
    let activeToken = localStorage.getItem('codementor_token');

    // If we just came back from GitHub, use the new token in the URL
    if (urlToken) {
      localStorage.setItem('codementor_token', urlToken);
      setToken(urlToken);
      activeToken = urlToken; // Update our activeToken variable

      // Clean up the URL
      window.history.replaceState({}, document.title, window.location.pathname);
    }

    // Now activeToken is properly defined and available to use!
    if (activeToken) {
      try {
  ...
  const handleGitHubLogin = () => {
    // Redirects the browser to your backend auth route
        ...
        <div className="max-w-6xl mx-auto">
          ...
          <header className="mb-10 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
            ...
            <div className="flex items-center gap-4">
              ...
              {/* NEW: Auth Buttons */}
              {/* UPDATED: User Profile & Auth Buttons */}
              {user ? (
                <div className="flex items-center gap-4 bg-white dark:bg-slate-800 p-1.5 pr-4 rounded-full border border-slate-200 dark:border-slate-700 shadow-sm">
                  <img
                    src={user.avatar}
                    alt="GitHub Avatar"
                  ...
                  <button
                    ...
                    className="text-xs font-bold text-slate-500 hover:text-red-500 dark:text-slate-400 dark:hover:text-red-400 transition-colors"
                  >
                    Logout
                  </button>
                </div>
                ...
                <button
                  ...
                  className="bg-[#24292e] hover:bg-[#1b1f23] text-white flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-bold transition-all shadow-sm"
                >
